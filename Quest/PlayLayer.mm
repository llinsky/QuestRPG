...
-(void) setCenterOfScreen: (CGPoint) position
{
    ...
    x = MIN(x, theHKMap.mapSize.width * theHKMap.tileSize.width - screenSize.width/2);
    y = MIN(y, theHKMap.mapSize.height * theHKMap.tileSize.height - screenSize.height/2);
...
- (BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (abs(diff.x) > abs(diff.y))
    {
        if (diff.x > 0)
        {
            ...
            playerPos.x += theHKMap.tileSize.width;
        ...
        if (diff.y > 0)
        {
            ...
            playerPos.y += theHKMap.tileSize.width;
    ...
    if ((playerPos.x <= (theHKMap.mapSize.width * theHKMap.tileSize.width)) && (playerPos.y <= (theHKMap.mapSize.height * theHKMap.tileSize.height)) && (playerPos.y >= 0) && (playerPos.x >= 0))
    {
...
- (CGPoint) tileCoordForPosition: (CGPoint) point
{
    ...
    int x = point.x/theHKMap.tileSize.width;
    int y = ((theHKMap.mapSize.height* theHKMap.tileSize.height) - point.y)/theHKMap.tileSize.height;
