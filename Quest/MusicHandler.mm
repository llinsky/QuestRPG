...
+(void) fadeBackgroundMusic:(double)fadeTime music:(int)musicCode
{
    if ([[SimpleAudioExtension sharedEngine] isBackgroundMusicPlaying])
     {
         [[SimpleAudioExtension sharedEngine] fadeBackgroundMusicFrom:[GameSceneManager sharedGameSceneManager].hero.musicVolume to:0.0 duration:fadeTime nextCode:musicCode];
     }
    else{
