...
- (id) init
{
    ...
    if (self != nil) {
		...
		self.musicSlider.value = floor([GameSceneManager sharedGameSceneManager].musicVolume*100.0); // floor(sae.backgroundMusicVolume * 100);
		...
		self.soundSlider.value = floor([GameSceneManager sharedGameSceneManager].soundVolume*100.0); //floor(sae.effectsVolume * 100);
...
- (void)onMusicSlide:(id)sender
{
    ...
    [GameSceneManager sharedGameSceneManager].musicVolume = (slider.value/100.0);
}
...
- (void)onMusicClick:(id)sender
{
    ...
    if ([sender selectedIndex] == 1) {
        ...
        musicMuted = TRUE;
        [GameSceneManager sharedGameSceneManager].musicVolume = 0;
        ...
        [GameSceneManager sharedGameSceneManager].musicVolume = ((float)prevMusicLevel / 100);
	}
...
- (void)onSoundSlide:(id)sender
{
    ...
    [GameSceneManager sharedGameSceneManager].soundVolume = (slider.value/100.0);
}
...
- (void)onSoundClick:(id)sender
{
    ...
    if ([sender selectedIndex] == 1) {
        ...
        [GameSceneManager sharedGameSceneManager].soundVolume = 0;
	}else {
		//button in on state, return to previous level
        ...
        [GameSceneManager sharedGameSceneManager].soundVolume = ((float)prevMusicLevel / 100);
	}
}
